...
data = 'real' #test/real
input = open(f'Day 21\star2\\{data}input.txt', 'r').read().split('\n')

sr, sc = next((r,c) for r, row in enumerate(input) for c, ch in enumerate(row) if ch == 'S')
...
def fill(sr, sc, ss):
    ...
    while q:
        ...
        for nr, nc in [(r + 1, c), (r-1, c), (r, c+1), (r, c-1)]:
            if nr < 0 or nr >= len(input) or nc < 0 or nc >= len(input[0]) or input[nc][nr] == '#' or (nr, nc) in seen:
                continue
            seen.add((nr, nc))
            q.append((nr, nc, s-1))
    return len(ans)
